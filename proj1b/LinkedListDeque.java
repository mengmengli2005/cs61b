public class LinkedListDeque<itemsType> implements Deque<itemsType> {
    ...
    public void addLast(itemsType x) {
        if (this.size() == 0) {
            IntNode<itemsType> newTail = new IntNode(sentinel, x, sentinel);
            sentinel.pre = newTail;
            sentinel.next = newTail;
            size = size + 1;
            return;
        }
        IntNode<itemsType> oldTail = sentinel.pre;
        IntNode<itemsType> newTail = new IntNode(oldTail, x, sentinel);
    ...
    public static void main(String[] args) {
        LinkedListDeque<Integer> A = new LinkedListDeque(); /* define link A */
